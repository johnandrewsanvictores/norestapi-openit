import React, { useMemo } from 'react';

const SeismicActivityChart = ({ earthquakes = [] }) => {
  const getQuakeTimestamp = (quake) => {
    if (typeof quake.timestamp === 'number') {
      return quake.timestamp;
    }
    if (typeof quake.time === 'number') {
      return quake.time;
    }
    if (quake.time || quake.timestamp) {
      const parsed = new Date(quake.time || quake.timestamp).getTime();
      if (!isNaN(parsed)) {
        return parsed;
      }
    }
    return 0;
  };

  const chartData = useMemo(() => {
    const now = Date.now();
    const sevenDaysAgo = now - (7 * 24 * 60 * 60 * 1000);
    
    const recentQuakes = earthquakes.filter(q => {
      const quakeTime = getQuakeTimestamp(q);
      return quakeTime >= sevenDaysAgo && quakeTime <= now;
    });

    console.log('SeismicActivityChart - Total earthquakes:', earthquakes.length);
    console.log('SeismicActivityChart - Recent (7 days):', recentQuakes.length);

    const dailyData = {};
    recentQuakes.forEach(quake => {
      const quakeTime = getQuakeTimestamp(quake);
      if (quakeTime === 0) {
        console.warn('SeismicActivityChart - Invalid timestamp:', quake);
        return;
      }
      
      const date = new Date(quakeTime);
      const dateKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
      
      if (!dailyData[dateKey]) {
        dailyData[dateKey] = { count: 0, magnitudes: [], date: dateKey, dateTime: date };
      }
      dailyData[dateKey].count++;
      dailyData[dateKey].magnitudes.push(parseFloat(quake.magnitude || 0));
    });

    const sortedData = Object.entries(dailyData)
      .map(([key, data]) => ({ ...data, key }))
      .sort((a, b) => a.key.localeCompare(b.key))
      .slice(-7); 

    console.log('SeismicActivityChart - Chart data points:', sortedData.length);
    console.log('SeismicActivityChart - Sample data:', sortedData[0]);

    return sortedData;
  }, [earthquakes]);

  const maxCount = chartData.length > 0 ? Math.max(...chartData.map(d => d.count), 1) : 1;

  return (
    <div className="bg-[#2A2A2A] rounded-lg p-6 border border-gray-800">
      <div className="mb-4">
        <h2 className="text-xl font-bold text-white mb-2">Seismic Activity - 7 Days</h2>
        <p className="text-sm text-gray-400">Earthquake frequency by day</p>
      </div>

      <div className="h-64 bg-[#1A1A1A] rounded-lg p-4 border border-gray-700">
        {chartData.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center">
            <p className="text-gray-500 text-sm mb-2">No earthquake data available for the last 7 days</p>
            <p className="text-gray-600 text-xs">Total earthquakes in dataset: {earthquakes.length}</p>
          </div>
        ) : (
          <div className="h-full flex items-end gap-2">
            {chartData.map((day, index) => {
              const heightPercent = (day.count / maxCount) * 100;
              const avgMagnitude = day.magnitudes.length > 0
                ? day.magnitudes.reduce((a, b) => a + b, 0) / day.magnitudes.length
                : 0;
              const magnitudeColor = avgMagnitude >= 5.0 ? 'bg-red-500' : 
                                    avgMagnitude >= 4.0 ? 'bg-[#FF7F00]' : 
                                    'bg-blue-500';
              
              const displayDate = day.dateTime ? day.dateTime.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : day.date;
              
              return (
                <div key={`${day.key}-${index}`} className="flex-1 flex flex-col items-center group relative min-w-[8px]">
                  <div
                    className={`w-full ${magnitudeColor} rounded-t transition-all hover:opacity-80 cursor-pointer`}
                    style={{ height: `${Math.max(heightPercent, 5)}%`, minHeight: '4px' }}
                    title={`${displayDate} - ${day.count} earthquake${day.count !== 1 ? 's' : ''}, Avg: ${avgMagnitude.toFixed(1)}`}
                  />
                  <div className="absolute bottom-full mb-2 hidden group-hover:block bg-[#2A2A2A] text-white text-xs px-2 py-1 rounded border border-gray-700 z-10 whitespace-nowrap">
                    <div>{displayDate}</div>
                    <div>Count: {day.count}</div>
                    <div>Avg: {avgMagnitude.toFixed(1)}</div>
                  </div>
                </div>
              );
            })}
          </div>
        )}
      </div>
    </div>
  );
};

export default SeismicActivityChart;
